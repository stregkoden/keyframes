...
local _ = function()
{
	if( !("_kfb283BZ1b8a" in this) )
	{
		...
		_kfb283BZ1b8a <- VS.CreateTimer(null, FTIME, null,null,false,true,true);

		// holds edit mode status
		_kfb283B2lbBa <- VS.CreateTimer(null, _kfb283B21b8a-FrameTime(), null,null,false,true,true);

		_kfb283B2tbBa <- VS.CreateTimer(null, FrameTime()*2.5, null,null,false,true,true);

		_kfB2B38Z1bBa <- VS.CreateEntity("game_text",{
			channel = 1,
		...
		_kfB2B38Z1bB4 <- VS.CreateEntity("game_text",{
			channel = 2,
			color = Vector(255,138,0),
			holdtime = _kfb283B21b8a,
			x = 0.56,
		...
		_kfB2B38ZlbB4 <- VS.CreateEntity("env_hudhint",null,true);

		HPlayerEye <- VS.CreateMeasure(HPlayer.GetName(),null,true);

		// holds compile status
		_kfB2B382lbB4 <- VS.CreateEntity("point_viewcontrol", { spawnflags = 1<<3 });

		// holds player noclip status
		_kfB2B3821bBA <- VS.CreateEntity("game_ui", { spawnflags = 1<<7, fieldofview = -1.0 },true);
		...
		_kfB2bEB2lbB4 <- VS.CreateEntity("env_sprite",
		{
...
function SetAngles(h,v)
{
	h.SetAngles(v.x,v.y,v.z);
}
...
function _kf82bE8Z1BBA()
{
	if( __roll_R )
	{
		...
		SetAngles(_kfB2B382lbB4, _kf82bE8Z1BB4);
...
function _kf82bE82lbBA(i=0)
{
	...
	if( _kf82bE821bBA )
	{
		...
		SetAngles(_kfB2B382lbB4, VS.QuaternionAngles2(__la_q[_kfb283821bBa]));
...
VS.OnTimer(_kfb283BZ1b8a, function()
{
	...
	::SetAngles(::_kfB2B382lbB4, __la_p[_kfB283B2lbB4]);
...
function _kfB283B2lbBA()
{
	...
	SetAngles(_kfB2B382lbB4,s.__la_p[0]);
	EntFireByHandle(_kfB2B382lbB4, "enable", "", 0, HPlayer);
